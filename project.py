...
st.set_page_config(page_title="Clasificación de Especies Iris", layout="wide")

# Título y descripción
st.title("🌸 Dashboard de Clasificación de Especies Iris")
st.markdown("""
Este proyecto implementa un flujo de trabajo de minería de datos completo para clasificar flores Iris.
Utiliza un modelo de **Random Forest** para predecir la especie basándose en las medidas de sépalos y pétalos.
""")
...
# --- 4. PREDICCIÓN ---
st.subheader("1. Predicción del Modelo")

# Realizar predicción con los datos del usuario
...
with col2:
    st.write("Probabilidad de predicción:")
    prob_df = pd.DataFrame(prediction_proba, columns=target_names)
    st.dataframe(prob_df.style.format("{:.2%}"))

# --- 5. MÉTRICAS DE EVALUACIÓN ---
st.subheader("2. Métricas del Modelo (Conjunto de Prueba)")
...
# --- 6. VISUALIZACIÓN 3D INTERACTIVA (CRÍTICO) ---
st.subheader("3. Visualización 3D: Dataset + Tu Muestra")
st.markdown("Este gráfico muestra la distribución de las especies. **El punto rojo grande** representa los valores que acabas de ingresar.")

# Preparamos los datos para el gráfico 3D
# 1. Dataset Original
plot_data = df.copy()
plot_data['Type'] = 'Dataset Original'
plot_data['Size'] = 2  # Tamaño normal

# 2. Dato del Usuario
user_point = input_df.copy()
user_point['species_name'] = 'INPUT USUARIO' # Etiqueta especial
user_point['Type'] = 'Input Usuario'
user_point['Size'] = 10 # Tamaño grande para destacar

# Concatenamos (Nota: 'species' columna numérica no es necesaria para el plot visual si usamos species_name)
...
# --- 7. EDA ADICIONAL ---
st.subheader("4. Análisis Exploratorio de Datos (EDA)")

tab1, tab2 = st.tabs(["Histogramas", "Matriz de Dispersión"])

with tab1:
    st.write("Distribución de las características por especie:")
    feature_to_plot = st.selectbox("Selecciona característica:", df.columns[:-2])
...
with tab2:
    ...
    # Usamos seaborn para pairplot estático, es muy claro para Iris
    fig_sns = sns.pairplot(df, hue="species_name", markers=["o", "s", "D"])
    st.pyplot(fig_sns)

st.markdown("---")
st.caption("Proyecto Final de Minería de Datos - Desarrollado con Streamlit")
